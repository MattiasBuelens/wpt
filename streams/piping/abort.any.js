...
for (const invalidSignal of [null, 'AbortSignal', true, -1, Object.create(AbortSignal.prototype)]) {
  promise_test(async t => {
    ...
    await promise_rejects_js(t, TypeError, rs.pipeTo(ws, { signal: invalidSignal }), 'pipeTo should reject');
...
promise_test(async t => {
  ...
  await promise_rejects_dom(t, 'AbortError', rs.pipeTo(ws, { signal }), 'pipeTo should reject');
  await Promise.all([
    rs.getReader().closed,
...
promise_test(async t => {
  ...
  const signal = abortController.signal;
  abortController.abort();
  await promise_rejects_dom(t, 'AbortError', rs.pipeTo(ws, { signal, preventCancel: true }), 'pipeTo should reject');
...
promise_test(async t => {
  ...
  const signal = abortController.signal;
  abortController.abort();
  await promise_rejects_dom(t, 'AbortError', rs.pipeTo(ws, { signal, preventAbort: true }), 'pipeTo should reject');
  assert_equals(ws.events.length, 0, 'writable should not have been aborted');
  await ws.getWriter().ready;
...
promise_test(async t => {
  ...
  const signal = abortController.signal;
  abortController.abort();
  await promise_rejects_dom(t, 'AbortError', rs.pipeTo(ws, { signal, preventCancel: true, preventAbort: true }),
...
promise_test(async t => {
  ...
  const signal = abortController.signal;
  abortController.abort();
  await promise_rejects_exactly(t, error2, rs.pipeTo(ws, { signal }), 'pipeTo should reject');
  assert_array_equals(events, ['abort', 'cancel'], 'abort() should be called before cancel()');
...
promise_test(async () => {
  ...
  abortController.abort();
  await pipeToPromise;
  await ws.getWriter().write('this should succeed');
...
promise_test(async t => {
  ...
  await Promise.resolve();
  abortController.abort();
  await promise_rejects_exactly(t, error1, pipeToPromise, 'pipeTo should reject');
  await ws.getWriter().write('this should succeed');
